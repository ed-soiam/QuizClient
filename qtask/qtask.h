...
public:
    ...
    QTask & operator =(const QTask & obj);
    virtual void setTaskData(const QByteArray & data) {_data = data;}
    QByteArray taskData() const {return _data;}
    //условие проверки активности задачи по внешней переменной
    void setConditionPointer(bool * pointer){_pre_check = pointer;}

    void setInfinity(bool value){_infinity = value;}
    ...
    void setRetryCount(int value){_retries = value;}
    int retryCount() const{return _retries;}
protected:
